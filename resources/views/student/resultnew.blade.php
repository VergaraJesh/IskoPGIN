    ...
    <table class="table table-bordered">
            ...
            @foreach($students as $users)
              <tr>
                ...
                <td><font face = "Times New Roman" size = "-1">{{ $users['lname'] }} </font></td>
                <td><font face = "Times New Roman" size = "-1">{{ $users['fname'] }}</font></td>
                
                <td>@foreach($users['mother'] as $gwa)
                                <font face = "Times New Roman" size = "-1">{{ ucwords($gwa)}}</font>
                         @endforeach</td>
                <td>@foreach($users['father'] as $gwa)
                                <font face = "Times New Roman" size = "-1">{{ ucwords($gwa) }}</font>
                         @endforeach</td>
                <td>@foreach($users['brgy'] as $gwa)
                                <font face = "Times New Roman" size = "-1">{{ $gwa }}
                        @endforeach,
                        @foreach($users['mun'] as $gwa)
                        {{ $gwa }}</font>
                        @endforeach
                </td>
              </tr>
